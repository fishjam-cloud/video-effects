// @ts-nocheck
/** CNN compute kernels compiled once during provider setup. */
import tgpu, { d, std } from "typegpu";

import { BroadcastFlag } from "../bundle";
import {
  activationSlot,
  binaryOpSlot,
  blockedPixel,
...
export const conv1x1Kernel = tgpu.computeFn({
  ...
  while (inBlock < shape.blocks.x) {
    const value =
      weightedLayout.$.src[
        hwc4Index(out.y, out.x, inBlock, shape.input.x, shape.blocks.x)
      ];
    const weightBase =
      weightedLayout.$.offsets.weightBase +
      (out.z * shape.blocks.x + inBlock) * 4;
...
const createConvKernel = (taps: readonly number[], kernelSize: number) =>
  tgpu.computeFn({
    ...
    const out = blockedPixel(i, shape.output.x, shape.blocks.y);
    let acc = d.vec4f(
      packedWeightAt(weightedLayout.$.offsets.biasBase + out.z),
    );
        ...
        for (const kx of tgpu.unroll(taps)) {
          ...
          if (!outOfBounds(inX, shape.input.x)) {
            ...
            while (inBlock < shape.blocks.x) {
              ...
              const weightBase =
                weightedLayout.$.offsets.weightBase +
                ((out.z * kernelSize + kernelY) * kernelSize + kernelX) *
                  shape.blocks.x *
                  4 +
                inBlock * 4;
              acc += packedDot4(value, weightBase);
...
export const globalPoolKernel = tgpu.computeFn({
  ...
  while (offset < pixels) {
    ...
    sum +=
      poolLayout.$.src[hwc4Index(y, x, block, shape.input.x, shape.blocks)];
  ...
  while (stride > 0) {
    if (localIndex < stride) {
      poolWorkgroupSums.$[localIndex] +=
        poolWorkgroupSums.$[localIndex + stride];
    }
    std.workgroupBarrier();
...
export const binaryKernel = tgpu.computeFn({
  ...
  let b = d.vec4f(
    binaryLayout.$.b[hwc4Index(bY, bX, bBlock, shape.bShape.x, shape.bShape.y)],
  );
  if ((shape.flags & BroadcastFlag.C) !== 0) {
    b = d.vec4f(b.x);
  }
    ...
    for (const kx of tgpu.unroll([0, 1])) {
      ...
      while (inBlock < shape.inputBlocks) {
        ...
        const weight = packedHeadWeightAt(
          headLayout.$.offsets.weightBase +
            spatial * shape.inputBlocks +
            inBlock,
        );
