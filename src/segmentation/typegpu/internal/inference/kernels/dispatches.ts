// @ts-nocheck
/** Compiles the parsed graph into specialized compute dispatches. */
    ...
    switch (record.opKind) {
      ...
      case OpKind.AvgPool: {
        ...
        const pipeline = specializedPipeline(
          ...
          () =>
            root
              .with(poolShapeSlot, shape)
              .createComputePipeline({ compute: globalPoolKernel }),
        );
        return {
          pipeline,
      ...
      case OpKind.Resize2x: {
        ...
        const pipeline = specializedPipeline(
          ...
          () =>
            root
              .with(resizeShapeSlot, shape)
              .createComputePipeline({ compute: resize2xKernel }),
        );
        return {
          pipeline,
          bindGroup: root.createBindGroup(poolLayout, {
      ...
      case OpKind.Mul: {
        ...
        const pipeline = specializedPipeline(
          [record.opKind, ...record.output, record.flags],
          () =>
            root
