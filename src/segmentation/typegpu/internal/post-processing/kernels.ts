// @ts-nocheck
/** Temporal smoothing, person prior, and edge-aware upsampling kernels. */
import tgpu, { d, std } from "typegpu";

import { initialFrameCropParams } from "../frame";
...
export const POST_PROCESS_WORKGROUPS = Math.ceil(
  MODEL_PIXELS / POST_PROCESS_WORKGROUP_SIZE,
);
...
const modelUvAt = (coord: d.v2i) => {
  "use gpu";
  return (
    (d.vec2f(std.clamp(coord, d.vec2i(0), MAX_MASK_COORD)) + 0.5) / MODEL_SIZE
  );
};
...
export const temporalAccumulatorKernel = tgpu.computeFn({
  ...
  const agreement =
    1 - std.smoothstep(AGREEMENT_LOW, AGREEMENT_HIGH, std.abs(raw - localMean));
  const supportScale = std.mix(0.55, 1, agreement);
  const confidentBackground = 1 - std.smoothstep(0.08, 0.2, raw);
  const alpha = std.max(
    certaintyBlend * supportScale,
    confidentBackground * BACKGROUND_FAST_ALPHA,
  );
  const filteredLogit = std.mix(
    temporalLayout.$.historyLogits[i],
    rawLogit,
    alpha,
  );
...
export const personCorePriorKernel = tgpu.computeFn({
  ...
  const attenuation = std.mix(
    1,
    OUTSIDE_CORE_ATTENUATION,
    outside * (1 - uncertaintyProtection),
  );
...
export const upsampleToTextureKernel = tgpu.computeFn({
  ...
  for (let dy = -UPSAMPLE_RADIUS; dy <= UPSAMPLE_RADIUS; dy++) {
    for (let dx = -UPSAMPLE_RADIUS; dx <= UPSAMPLE_RADIUS; dx++) {
      ...
      const spatialWeight = gaussianWeight(
        spatialDist2,
        UPSAMPLE_SPATIAL_SIGMA,
      );
