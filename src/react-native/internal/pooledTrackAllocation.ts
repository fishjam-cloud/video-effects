...
function toWorkletBufferDescriptor(
  buffer: CustomVideoBuffer,
): WorkletBufferDescriptor {
  return {
    index: buffer.index,
  ...
  for (let attempt = 1; attempt <= POOL_DISPOSE_ATTEMPTS; attempt += 1) {
    try {
      ...
      if (!isPoolInUseError(cause) || attempt === POOL_DISPOSE_ATTEMPTS) {
        console.warn(
          "pooledTrackAllocation: disposing the buffer pool failed",
          cause,
        );
        return;
      }
      await new Promise((resolve) =>
        setTimeout(resolve, POOL_DISPOSE_RETRY_MILLISECONDS),
      );
    }
  }
}
...
export function disposeAllocation({
  pool,
  stream,
}: PooledTrackAllocation): void {
