...
export interface WebGpuFrameRenderer {
   ...
   */
  renderFrame: (
    input: WebGpuCameraFrameInput,
    onFrame: (render: WebGpuFrameRenderFunction) => void,
  ) => void;
}
 ...
 * either changes.
 */
export function createWebGpuFrameRenderer(
  options: CreateWebGpuFrameRendererOptions,
): WebGpuFrameRenderer {
  ...
  const renderFrame = (
    ...
    "worklet";
    const videoFrame = runtime.createVideoFrameFromNativeBuffer(
      input.nativeBuffer,
    );
    try {
      const isRotatedQuarterTurn =
        input.rotationDegrees === 90 || input.rotationDegrees === 270;
      const cameraWidth = isRotatedQuarterTurn
        ? videoFrame.height
        : videoFrame.width;
      const cameraHeight = isRotatedQuarterTurn
        ? videoFrame.width
        : videoFrame.height;
      ...
      try {
        ...
        const render: WebGpuFrameRenderFunction = (encode) => {
          ...
          if (rendered) {
            throw new Error(
              "createWebGpuFrameRenderer: render() may only be called once per frame.",
            );
          }
          rendered = true;
          ...
          if (imported == null) {
            const memory = device.importSharedTextureMemory({
              handle: descriptor.surfaceHandle,
            });
