...
const sampleRgbCamera: SampleCameraFn = tgpu
  .fn(
    [d.vec2f],
    d.vec4f,
  )(SAMPLE_RGB_CAMERA)
  .$name("sampleCamera");
const sampleYcbcrRawCamera: SampleCameraFn = tgpu
  .fn(
    [d.vec2f],
    d.vec4f,
  )(SAMPLE_YCBCR_RAW_CAMERA)
  .$name("sampleCamera");
 ...
 * @group WebGPU
 */
export function sampleCameraForPixelLayout(
  cameraPixelLayout: CameraPixelLayout,
): SampleCameraFn {
  return cameraPixelLayout === "ycbcr-raw"
    ? sampleYcbcrRawCamera
    : sampleRgbCamera;
}
