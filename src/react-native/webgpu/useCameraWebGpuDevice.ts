...
async function acquireSharedCameraWebGpuDevice(): Promise<GPUDevice> {
  ...
  if (adapter == null) {
    throw new Error(
      "WebGPU is unavailable: navigator.gpu.requestAdapter() returned no adapter.",
    );
  }
...
export function getCameraWebGpuDevice(): Promise<GPUDevice> {
  if (sharedDevicePromise == null) {
    ...
    const devicePromise: Promise<GPUDevice> =
      acquireSharedCameraWebGpuDevice().then(
        (device) => {
...
export function useCameraWebGpuDevice(): UseCameraWebGpuDeviceResult {
  const [result, setResult] = useState<UseCameraWebGpuDeviceResult>({
    device: null,
    error: null,
  });
 ...
 */
export function useCameraWebGpuDeviceWithOverride(
  override: GPUDevice | undefined,
