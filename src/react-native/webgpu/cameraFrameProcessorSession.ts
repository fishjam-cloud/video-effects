...
} from "../internal/pooledTrackAllocation";
...
export async function createCameraFrameProcessorSession(
  ...
  try {
    ...
    if (disposed) {
      ...
      throw new Error(
        "createCameraFrameProcessorSession: disposed before setup finished.",
      );
    }
    ...
    if (publishedTrack == null) {
      ...
      disposeAllocation(allocation);
      throw new Error(
        "createCameraFrameProcessorSession: the pooled stream has no video track.",
      );
    }
