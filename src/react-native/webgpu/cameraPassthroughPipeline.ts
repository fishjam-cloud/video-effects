...
const cropLayout = tgpu.bindGroupLayout({
  cropParams: { uniform: FrameCropParams },
});
...
function makeFragmentMain(mirror: boolean, sampleCamera: SampleCameraFn) {
  ...
  return tgpu
    .fragmentFn({ in: { screenUv: d.location(0, d.vec2f) }, out: d.vec4f })(
      (input) => {
        ...
        const cropUv = add(
          mul(input.screenUv, d.vec2f(mirrorSign, 1)),
          d.vec2f(mirrorOffset, 0),
        );
        ...
        const cameraUv = add(
          mul(cp.uvTransform, sub(sourceUv, d.vec2f(0.5))),
          d.vec2f(0.5),
        );
 ...
 */
function buildPassthroughShaderCode(
  ...
  const resolved = tgpu.resolve(
    [vertexMain, makeFragmentMain(mirror, cameraShaderBindings.sampleCamera)],
    {
      names: "strict",
    },
  );
...
export function createCameraPassthroughPipeline(
  ...
  bindGroupLayouts[CAMERA_BIND_GROUP_INDEX] =
    cameraShaderBindings.bindGroupLayout;
